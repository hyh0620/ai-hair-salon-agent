...
* Python 3.12；
* 338 个自动化测试，0 failures，0 warnings；
...
登录和注册入口使用当前应用进程内的有界滑动窗口限流。登录同时限制单个直连客户端的总请求频率，以及同一客户端与同一规范化邮箱的组合频率；注册限制单个直连客户端的请求频率。被限制的请求在用户查询、Argon2 和数据库写入前返回 HTTP 429，并提供 `Retry-After`。
...
当前账户能力是可信预约 owner 的认证 MVP，不是完整生产身份系统。浏览器退出只清除 Cookie，不会立即吊销此前复制的 Bearer Token；当前进程内限流也不跨 Worker 或实例共享。生产部署还需要 HTTPS、secret 生命周期管理、分布式限流、审计和更完整的账号安全策略。项目不声称已经完成生产部署。
