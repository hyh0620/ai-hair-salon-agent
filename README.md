...
系统同时提供可选本地账户：密码使用 Argon2 哈希，短期 Access JWT 绑定可吊销的服务端认证会话，浏览器通过 HttpOnly Cookie 自动轮换登录凭据，API 客户端可使用 Bearer Token。未登录时仍可使用游客预约模式。
...
* Python 3.12；
* 403 个自动化测试，0 failures，0 warnings；
...
* [5 分钟核心演示](docs/DEMO_GUIDE.md)：默认从业务问题、核心流程和架构边界开始；
* [系统架构与技术边界](docs/ARCHITECTURE.md)：用于事务、身份、Session、MCP/RAG 等技术追问；
* [测试与 RAG 评估](docs/EVALUATION.md)：区分功能契约、自动化回归和检索质量；
* [本地运行与深度演示 Runbook](docs/DEMO_RUNBOOK.md)：用于安装、数据准备、认证深挖和故障复现。

面试现场默认先看 5 分钟演示；需要技术深挖时进入架构与评估文档，环境搭建和故障复现则使用 Runbook。
...
`AuthService` 使用 `pwdlib` 的 Argon2 实现注册与密码验证，并使用 PyJWT 签发固定为 HS256 的短期 Access Token。JWT 包含内部 `sid`，每次认证请求除校验签名、issuer、audience 和 expiry 外，还会检查 SQLite 中对应的 Auth Session 是否属于当前用户、未撤销且未过期。默认 Access Token 有效期为 15 分钟；旧版本中不含 `sid` 的 JWT 升级后会失效，用户需要重新登录一次。

Refresh Token 是使用 `secrets` 生成的高熵不透明随机值，只放入 Path 为 `/api/auth` 的 HttpOnly Cookie；数据库仅保存 SHA-256 Hash，响应 JSON 和 JavaScript 均不可读取原值。`POST /api/auth/refresh` 使用 Cookie Jar 与双提交 CSRF，原子消费旧 Token 并签发新 Access/Refresh Token，不提供 JSON Body Refresh Token 流程，也不会延长 Auth Session 的 30 天绝对寿命。

同一个 Refresh Token 在 3 秒 Grace Window 内的并发重复返回 409，不再次轮换；窗口外重放会撤销受影响的整个 Auth Session。Logout 会撤销当前服务端 Session，因此该 Session 下已复制的 Bearer Token 立即失效；同一账户的其他登录 Session 保持有效。浏览器在普通同源请求收到 401 时采用 single-flight 刷新，成功后最多重试原请求一次。

登录、注册和刷新入口使用当前应用进程内的有界滑动窗口限流。登录同时限制单个直连客户端的总请求频率，以及同一客户端与同一规范化邮箱的组合频率；注册和刷新限制单个直连客户端的请求频率。被限制的请求在用户查询、Argon2 或认证 Session 数据库查询前返回 HTTP 429，并提供 `Retry-After`。

客户端范围只来自 `request.client.host`，不信任 `X-Forwarded-For`、`Forwarded` 或 `X-Real-IP`。部署在反向代理后，需要显式配置可信代理链，或在反向代理/API Gateway 层执行限流。当前 limiter 状态仅保存在单个进程内，应用重启后清空，多 Worker 或多实例之间不共享；生产环境仍需要 Redis、API Gateway 或反向代理提供分布式限流，不能把当前实现视为完整生产防护。
...
用户行为分析与回访提醒复用同一个 `RequestIdentity`。登录账户按 `account:<user_uuid>` 隔离，游客按当前浏览器的 `anonymous_owner_id` 隔离；客户端 `user_id` 不能选择分析范围，`X-Anonymous-Owner-ID` 也不能覆盖已验证账户。无效 JWT 返回 401，不会降级为游客。页面只展示账户显示名称或“游客”，不展示内部 owner、UUID 或 Token。

游客历史不会在登录后自动合并到账户，当前也不提供游客历史认领功能。退出登录后，浏览器仍可使用原匿名 owner 返回游客数据范围。
...
* Auth Session 是服务端持久化登录状态，由 Access JWT 的内部 `sid` 引用，不展示给用户，也不参与预约 owner；
...
登录后，预约 owner 不再来自浏览器存储，而是由后端根据 JWT `sub` 生成 `account:<user_uuid>`。`sid` 仅定位可吊销的 Auth Session，不是 `chat_session_id`，也不会改变 `RequestIdentity.owner_id`。注册、登录和退出都会轮换 `chat_session_id`，刷新不会；`anonymous_owner_id` 不会被删除，退出后仍可返回原游客空间。
...
> Auth Session 管理登录凭据的有效性，`chat_session_id` 管理短期对话状态，`anonymous_owner_id` 管理可被客户端伪造的游客业务范围；三者互不替代。

Chat Session 本身不是登录凭证，也不是持久化 Memory 或 Redis 分布式 Session。应用重启后进程内对话状态不会保留；Auth Session 和 Refresh Hash 则持久化在 SQLite。浏览器保存的匿名 owner 仍可用于管理 SQLite 中已有游客预约，且不会在登录后自动转移到账户。
...
| 可选账户身份 | Argon2、短期 JWT `sid`、服务端 Auth Session、Refresh 轮换、HttpOnly Cookie 与 CSRF |
...
主要开发、运行和 CI 版本为 Python 3.12。完整环境安装、私有 Provider 配置和知识服务数据准备见[本地运行与深度演示 Runbook](docs/DEMO_RUNBOOK.md)。公开示例配置不包含真实 LLM 或 Embedding 凭据，RAG MCP 默认关闭，因此 Booking 与本地页面可以先独立启动。
...
账户功能需要在本地 `.env` 中为 `AUTH_JWT_SECRET` 设置至少 32 个随机字节，并保持该文件不进入 Git。未配置有效 secret 时，账户 API 返回 503，游客预约仍可使用；HTTPS 部署还应设置 `AUTH_COOKIE_SECURE=true`。浏览器 Refresh 使用 HttpOnly Cookie 和 CSRF；Swagger/CLI 登录返回的 Bearer Token 仍可直接使用，Cookie Refresh 需要支持 Cookie Jar。
...
当前账户能力是可信预约 owner 的认证 MVP，不是完整生产身份系统。服务端 Auth Session 支持当前会话吊销、一次性 Refresh 轮换和重放响应，但当前进程内限流不跨 Worker 或实例共享，SQLite 也不是多实例身份存储。生产部署仍需要 HTTPS、secret 生命周期管理、PostgreSQL/Redis 或独立身份服务、分布式限流、审计和更完整的账号安全策略。项目不声称已经完成生产部署。
...
* [本地运行与深度演示 Runbook](docs/DEMO_RUNBOOK.md)
...
v1.0 发布材料和验收记录包括自动化发布门禁、真实 Provider 端到端验收、演示流程与正式 Release Notes。正式版本和发布时间以 GitHub Releases 页面为准。

Hermetic CI 禁止真实外部调用，用于验证可复现的业务、事务、认证和故障边界；真实 Provider 验收在显式允许外部调用的隔离流程中执行，不属于 Hermetic CI。已完成的真实 Provider 验收确认：
