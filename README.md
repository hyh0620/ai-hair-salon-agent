...
评估语料包括 7 份源文档和 24 个语义切片。以上数字来自此前保存的历史验证结果，不是本次 pytest 或当前环境的现场输出。新的公开快照只有在 28 条用例全部使用真实服务执行、没有跳过项且通过逐用例重算后才会生成。详见[评估方法与结果](docs/EVALUATION.md)。
