...
class UserBehaviorRepository(BaseUserBehaviorRepository):
    ...
    def get_user_statistics(self, user_id: str, days_back: int = 30) -> Dict[str, Any]:
        ...
        with self.session_manager.session_scope() as session:
            ...
            favorite_stylist = session.query(
                UserBehavior.stylist_id,
                Stylist.name,
                func.count(UserBehavior.stylist_id).label('count')
            ).join(Stylist).filter(
                UserBehavior.user_id == user_id,
                UserBehavior.action_type == 'appointment',
                UserBehavior.created_at >= cutoff_date
            ).group_by(UserBehavior.stylist_id, Stylist.name).order_by(
                func.count(UserBehavior.stylist_id).desc()
            ).first()
            ...
            return {
                ...
                'favorite_stylist_id': favorite_stylist[0] if favorite_stylist else None,
    ...
    def get_stylist_popularity(self, days_back: int = 30) -> List[Dict[str, Any]]:
        """
        获取发型师受欢迎程度统计
        ...
        with self.session_manager.session_scope() as session:
            ...
            popularity = session.query(
                UserBehavior.stylist_id,
                Stylist.name,
                func.count(UserBehavior.stylist_id).label('appointment_count'),
    ...
    def _behavior_to_dict(self, behavior: UserBehavior) -> Dict[str, Any]:
        ...
        return {
            ...
            'stylist_id': behavior.stylist_id,
            'stylist_name': behavior.stylist.name if behavior.stylist else None,
    ...
    def _recommendation_to_dict(self, recommendation: UserRecommendation) -> Dict[str, Any]:
        ...
        return {
            ...
            'stylist_id': recommendation.stylist_id,
            'stylist_name': recommendation.stylist.name if recommendation.stylist else None,
