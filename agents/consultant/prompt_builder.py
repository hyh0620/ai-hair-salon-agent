...
class PromptBuilder:
    ...
    def _create_system_prompt(self) -> str:
        """创建系统提示词"""
        return (
            "你是一个理发店的前台接待员，负责为客户解答关于剪发、染发、烫发、造型、头皮护理、预约、价格、营业时间、地址、交通等相关问题。"
            ...
            "请用专业、礼貌、简洁的语言回复用户。"
            "如果用户的问题与理发店服务完全无关（如天气、股票、新闻等），请礼貌地告知用户你只能回答理发店相关问题。"
    ...
    def _create_classification_prompt_template(self) -> str:
        """创建分类提示词模板"""
        return (
            "你是一个分类器，判断用户输入是否是关于理发店的咨询类问题。\n"
            "咨询类问题包括：剪发、染发、烫发、造型、头皮护理、价格、营业时间、服务项目、发型师情况、店铺地址、交通路线、联系方式、店铺环境等。\n"
    ...
    def _build_knowledge_context(self, knowledge_docs: List[Dict[str, Any]]) -> str:
        ...
        if not knowledge_docs:
            return "没有找到直接相关的知识库信息，请基于你对理发店服务的一般了解回答。"
        
        context = "\n以下是相关的知识库信息：\n"
        for i, doc in enumerate(knowledge_docs, 1):
            context += f"{i}. {doc['content']}\n"
        context += "\n请基于以上信息回答用户问题。如果知识库信息不足以回答问题，请基于你对理发店服务的一般了解来补充回答。\n"
