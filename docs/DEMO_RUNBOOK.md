...
本文面向维护者和需要深入了解技术实现的演示者，用于复现环境、外部服务、知识库、认证会话、故障边界和天气后处理。默认 5 分钟流程见[项目演示指南](DEMO_GUIDE.md)，无需在演示时执行本文全部步骤。

## 1. 使用边界

* 使用可控的演示数据，不读取或展示真实用户数据；
* `.env`、外部服务密钥、JWT 密钥、令牌、Cookie、SQLite 数据库和知识索引不得提交；
* 普通 pytest 与 CI 必须保持隔离，不调用真实外部服务；
* 真实外部服务验收使用独立、显式流程，不等同于 CI；
...
从知识服务提供的示例配置创建本地配置，并按其仓库文档设置 Embedding 外部服务。真实凭据只能保存在私有环境中，不能写入命令、截图、日志或 Git：
...
知识导入（Ingest）会写入知识服务自己的 ChromaDB 和 BM25 目录。正常查询和默认项目演示不应重新导入，也不应修改或提交这些运行时数据。
...
`RAG_MCP_SERVER_PYTHON` 必须指向知识服务自己的虚拟环境。公开 `.env.example` 默认 `RAG_MCP_ENABLED=false`，所以预约功能和本地页面可以先独立运行。

## 8. FastAPI 与 MCP stdio 生命周期

完成 MCP 路径、模块、工作目录和知识集合（Collection）配置后，只需启动 FastAPI：
...
FastAPI 应用生命周期（lifespan）会自动：

1. 创建 `MCPKnowledgeGateway`；
2. 通过官方 MCP Python SDK 启动 stdio 子进程；
3. 执行 `initialize` 和 `list_tools`；
4. 验证 `query_knowledge_hub`；
5. 在知识咨询请求之间复用会话；
6. 应用关闭时清理会话和子进程。

正常集成不要求先手动常驻启动 MCP 服务端。手动运行 `python -m src.mcp_server.server` 仅用于单独验证 stdio 协议；终端没有交互提示并不代表服务失败。
...
状态页面适合确认配置，不应展示外部服务密钥、令牌或数据库路径。
...
冲突演示应复用刚刚成功创建的同一发型师和时间，并使用另一个测试预约归属发起请求。预期第二次写入被拒绝；不要硬编码未来会过期的时间。

## 11. RAG 与引用来源演示准备

确认 `/health` 中知识服务可用后，在聊天或知识咨询 API 提问：
...
MCP 是调用协议，向量检索、BM25 和 RRF 是知识服务内部的 RAG 链路。RAG 不决定价格、排班、冲突或预约成功。
...
该脚本使用临时 SQLite 和测试认证配置，并拒绝真实外部服务。可以展示：

* 注册或登录后获得服务器验证的账户预约归属；
* `/api/auth/me` 返回当前账户的安全显示信息；
* 账户与游客预约范围相互隔离；
* 认证会话与 `chat_session_id` 分别管理凭据状态和短期对话状态。

不要在终端或浏览器开发工具中展示令牌、Cookie、哈希、JWT 密钥、密码或原始邮箱。

## 13. 凭据刷新、退出登录、重放和多认证会话

这些流程不属于默认 5 分钟演示。深度验证应在隔离环境中完成，并优先引用自动化测试结果：

1. 访问令牌过期后，浏览器通过单请求合并刷新，最多重试原请求一次；
2. 刷新令牌轮换后，宽限窗口内的并发重复返回 409；
3. 宽限窗口外的重放撤销当前认证会话；
4. 退出登录撤销当前认证会话，已复制的 Bearer 令牌随即失效；
5. 同一账户的另一个认证会话保持可用。
...
* 知识咨询返回 HTTP 503 和稳定原因 `mcp_rag_unavailable`；
* 预约功能仍可查询候选并完成确定性流程；
* 故障实例退出后删除临时目录和数据库；
* 不把故障配置带回正常演示环境。

仓库中的 `eval/mcp_runtime_failure_e2e.py` 用于验证该契约。不要把 pytest 模拟结果描述为真实 MCP 集成结果。
...
隔离式测试在应用导入前设置 `EXTERNAL_CALL_POLICY=deny`，使用临时 SQLite、模拟 LLM、模拟 MCP 服务、模拟天气服务和固定时间。它不读取真实 `.env`，也不等同于真实外部服务验收。
...
### 预约功能正常，但知识咨询返回 HTTP 503

检查 `RAG_MCP_ENABLED`、知识服务解释器、模块、工作目录和知识集合（Collection）。正常集成由 FastAPI 应用生命周期（lifespan）自动拉起子进程，不需要另开终端常驻启动 MCP。

### 知识咨询可用，但没有返回引用来源

确认已导入目标知识集合，查询调用的是 `query_knowledge_hub`，并检查知识服务自身的向量检索、BM25 和 RRF 配置。不要手工拼接来源来伪造引用。
