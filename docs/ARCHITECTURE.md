...
> `chat_session_id` 管理短期交互状态，`anonymous_owner_id` 管理匿名业务所有权；两者都不是安全认证身份。
...
当前 REST `owner_id` 来自客户端提交的 `user_id`，聊天 owner 来自浏览器 localStorage 中独立保存的 `anonymous_owner_id`。它们没有经过登录认证，理论上可以被伪造，因此这是业务 owner 范围校验，不是安全意义上的身份隔离，也不提供跨设备同步。

生产环境应从 JWT、OAuth 或企业身份系统提供的可信认证上下文取得身份，以可信 `user_id` 替代 `anonymous_owner_id`，并忽略客户端自行声明的身份。同时需要引入共享 Session、服务型数据库、可观测性和审计能力；当前项目不声称已经具备这些生产基础设施。
