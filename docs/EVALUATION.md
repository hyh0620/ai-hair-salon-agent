...
* `eval/dataset_resolver.py`：为一次评估统一解析未来工作日；
* `eval/run_evaluation.py`：真实 API 评估执行器；
* `eval/evaluation_metrics.py`：功能契约和来源级检索指标；
* `eval/report_generator.py`：摘要和逐用例报告生成器；
* `eval/verified_snapshot.py`：生成去敏的已验证快照；
* `eval/verify_snapshot.py`：从逐用例明细重新计算并验证快照；
...
基准用例集（Golden Dataset）共 28 条。预约用例使用 `{{EVAL_DATE_DAY_1}}` 和 `{{EVAL_DATE_DAY_2}}` 占位符；执行器在单次运行开始时统一解析为未来工作日，确保 setup、正式请求和报告使用同一日期基准。
...
RAG 指标检查预期来源是否出现在知识服务返回结果和引用来源（Citations）中。同一 PDF 的多个 chunk 会先按公开文件名去重，再计算该来源第一次出现的排名：
...
以上数字来自 2026-07-24 使用真实 LLM、Embedding、MCP/RAG 和隔离故障模式服务完成的评估：

* [公开摘要](../eval/snapshots/verified_20260724_4bbe6d6d.md)；
* [逐用例 JSON](../eval/snapshots/verified_20260724_4bbe6d6d.json)；
* 关联评估代码提交：`4bbe6d6d`；
* 28 条用例全部执行，0 条跳过，功能契约 28 / 28。

新的已验证评估快照（Verified Evaluation Snapshot）只会在以下条件全部满足时写入 `eval/snapshots/`：

* 28 条用例全部执行且没有跳过项；
* 功能契约全部通过；
* 真实 LLM、Embedding、MCP/RAG 和故障模式服务均可用；
* Git 工作区干净；
* 快照通过 `eval/verify_snapshot.py` 重新计算 dataset count、Hit@1、Hit@3、MRR 和全部比例字段。
...
如需固定某轮评估使用的未来工作日，可额外传入：

```bash
--evaluation-base-date YYYY-MM-DD
```
