...
class UserBehaviorService:
    ...
    def analyze_user_patterns(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 分析偏好发型师
            preferred_stylist = self._analyze_preferred_stylist(appointment_behaviors)
    ...
    def _analyze_preferred_stylist(self, appointment_behaviors: List[Dict[str, Any]]) -> Optional[str]:
        """分析偏好发型师"""
        stylist_counts = {}
        
        for behavior in appointment_behaviors:
            stylist_id = behavior.get('stylist_id')
            if stylist_id:
                stylist_counts[stylist_id] = stylist_counts.get(stylist_id, 0) + 1
        
        if not stylist_counts:
            return None
        
        # 返回预约次数最多的发型师
        most_frequent_stylist = max(stylist_counts, key=stylist_counts.get)
        return most_frequent_stylist if stylist_counts[most_frequent_stylist] > 1 else None
